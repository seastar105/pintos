...
load (const char *file_name, void (**eip) (void), void **esp) 
{
 ...
 // printf("loading process : %s\n",t->name);
  ...
  int argc = 0;
  char *argv[128];
  for(token = strtok_r((char*)file_name," ",&ptr);
  		token!=NULL;
...
//  printf("309\n");
  ...
  /* start modification by Kwon Myung Joon */
	void *cur = *esp;
	char **argv_address = (char**)malloc(sizeof(char*) * 128);
	int total=0;
	i = argc;
	// push args
	while(i--) {
		...
		memcpy(cur, argv[i] , strlen(argv[i]) + 1);
		argv_address[i] = cur;
		//	  printf("%p\n",argv_address[i]);
	}
	//printf("after push args %p\n",cur);
	...
	while(i--) {
		cur -= sizeof(char*);
		memcpy(cur,&(argv_address[i]),sizeof(char*));
		//printf("%p\n",*(char**)cur);
	}
	// **argv
	argv_address[argc] = cur;
	cur -= sizeof(char**);
	memcpy(cur,&(argv_address[argc]),sizeof(char*));
	free(argv_address);
	// argc
	cur -= sizeof(int);
	memcpy(cur, &argc, sizeof(int));
	// return address
	cur -= sizeof(void (*));
